        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2 mt-1 sm:mt-0 sm:shrink-0">
            ...
            <button
              ...
              className={`flex items-center space-x-1.5 rounded-full border px-3.5 py-1.5 text-xs font-medium outline-none transition-all duration-150 focus-visible:ring-2 focus-visible:ring-blue-500/40 ${
                hasLiked
                  ? 'border-[#fad2cf] bg-[#fce8e6] text-red-500 hover:bg-[#fadad7]'
      ...
      <div className="space-y-4 sm:space-y-5 rounded-2xl bg-white p-4 sm:p-5 shadow-[0_1px_2px_0_rgba(60,64,67,0.3),0_1px_3px_1px_rgba(60,64,67,0.15)]">
        ...
        <form onSubmit={handleCommentSubmit} className="flex items-center gap-2 sm:gap-2.5">
          <input
            ...
            className="flex-1 w-full min-w-0 rounded-full border border-[#dadce0] bg-white px-3 py-2 sm:px-4 sm:py-2.5 text-[13px] sm:text-sm text-[#202124] outline-none transition-all duration-150 hover:border-[#c4c7cc]  focus:ring-2 focus:ring-blue-500/20  disabled:opacity-70"
            aria-label="Add comment input"
          />
          <button
            type="submit"
            disabled={!commentText.trim() || commentLoading}
            className="flex h-9 w-9 sm:h-10 sm:w-10 shrink-0 items-center justify-center rounded-full bg-blue-500 text-white shadow-sm outline-none transition-all duration-150 hover:bg-[#1765cc] hover:shadow-md focus-visible:ring-2 focus-visible:ring-blue-500/40 disabled:cursor-not-allowed "
            ...
            optimisticComments.map((c) => (
              <div key={c.id} className="space-y-1 py-3.5 first:pt-0">
                <div className="flex items-center gap-2 text-[10px] text-[#5f6368]">
                  <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[#e8f0fe] text-[10px] font-medium text-blue-500">
