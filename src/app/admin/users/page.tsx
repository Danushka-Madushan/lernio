...
const UsersAdminPage = () => {
    ...
    (list: Student[]) => {
      ...
      return list.filter((s) => {
        ...
        const matchTeacher = teacherFilter
          ? s.teacherId === teacherFilter || s.teacher?.id === teacherFilter
          : true;
  ...
  useEffect(() => {
    ...
    fetchStudents();
  }, [fetchRoleAndTeachers, fetchStudents]);
  ...
  const handleCreateStudent = async (e: React.FormEvent) => {
    ...
    if (!/^\d{4}$/.test(suffix)) {
      setError('Please fill in all 4 student ID digits');
      return;
    }
    if (!password) {
    ...
    try {
      const res = await fetch(`/api/users/${editTarget.id}`, {
        ...
        body: JSON.stringify({
          ...
          accessMode,
          grade: grade || null,
          ...(isAdmin && teacherId !== undefined ? { teacherId } : {}),
        }),
      });
      if (res.ok) {
      ...
      setError('Connection error updating account');
      setEditTarget(null);
    } finally {
  ...
  const handleSaveCustomVideos = async (videoIds: string[]) => {
    ...
    try {
      ...
      setError('Connection error saving custom video list');
      setCustomVideoTarget(null);
    }
  };
  ...
  const renderStudentRow = (student: Student) => (
    <tr key={student.id} className="transition-colors duration-100 hover:bg-[#f8f9fa]">
      ...
      <td className="py-3.5">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <span className="block font-mono text-[13px] font-medium text-[#202124] truncate">
              {student.username}
            </span>
            <span className="block text-[11px] text-[#9aa0a6]">
              {student.accessMode === 'CUSTOM' ? (
                ...
                <span className="inline-flex items-center gap-0.5 text-[#5f6368]">
                  <BookOpen size={9} /> Grade
                </span>
              )}
            </span>
          </div>
      ...
      {isAdmin && (
        <td className="py-3.5">
          {student.teacher ? (
            <span className="inline-flex items-center gap-1 rounded-full bg-purple-50 px-2.5 py-1 text-[11px] font-medium text-purple-700 border border-purple-100">
              <UserCheck size={11} /> {student.teacher.username}
            </span>
          ) : (
      ...
      <td className="py-3.5 text-right">
        <div className="inline-flex items-center gap-1">
          {student.accessMode === 'CUSTOM' && (
            <button
              type="button"
              onClick={() => setCustomVideoTarget(student)}
              title="Manage Videos"
  ...
  return (
    <>
      ...
      {showAddModal && isAdmin && (
        <AddStudentModal
          usernamePrefix={newUsernamePrefix}
          usernameSuffix={newUsernameSuffix}
      ...
      <div className="min-h-screen bg-[#f8f9fa] px-4 py-8 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h1 className="text-[22px] font-medium tracking-tight text-[#202124]">
                {isAdmin ? 'Student Accounts' : 'My Students'}
              </h1>
          ...
          <div className="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-3">
            <StatCard
              label={isAdmin ? 'Total Students' : 'My Students'}
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex items-center gap-2.5">
                    <h2 className="text-[15px] font-medium text-[#202124]">
                      {isAdmin ? 'Existing Student Accounts' : 'My Assigned Students'}
                    </h2>
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-44 sm:w-48 rounded-full border border-[#dadce0] bg-white py-2 pl-9 pr-8 text-sm text-[#202124] outline-none transition-all hover:border-[#c4c7cc] focus:ring-2 focus:ring-blue-500/20"
                      />
                      {searchQuery && (
                        <button
                          type="button"
                ...
                <div className="rounded-2xl w-full min-w-0 bg-white shadow-[0_1px_2px_0_rgba(60,64,67,0.3),0_1px_3px_1px_rgba(60,64,67,0.15)] overflow-hidden">
                    ...
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-[#9aa0a6]">
                          {expiredStudents.length} account{expiredStudents.length !== 1 ? 's' : ''} need reactivation
                        </p>
                      </div>
                    </div>
                    <ChevronDown
                      size={16}
                      className={`text-[#5f6368] transition-transform duration-200 ${
                  ...
                  {showExpired && (
                    filteredExpired.length === 0 ? (
                      <div className="px-6 pb-6 text-sm text-[#9aa0a6]">
                        No expired accounts match your filters.
                      </div>
                    ) : (
                        ...
                        <table className="w-full border-collapse text-left text-sm">
                          ...
                          <tbody className="divide-y divide-[#f1f3f4]">
                            {filteredExpired.map((student) => (
                              <tr
                                key={student.id}
                                className="transition-colors duration-100 hover:bg-[#fce8e6]/30"
                              >
                                <td className="py-3.5">
                                  <div className="flex items-center gap-3">
                                ...
                                <td className="py-3.5 text-[12px] text-[#9aa0a6]">
                                  {formatDate(student.activeTo)}
                                </td>
                                <td className="py-3.5 text-right">
                                  <div className="inline-flex items-center gap-1">
                                    {isAdmin ? (
